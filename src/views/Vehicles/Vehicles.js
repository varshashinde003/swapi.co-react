...
class Vehicles extends React.Component {
    ...
    async componentDidMount() {
        try {
            const data = await Service.getData("vehicles", this.state.page)
    ...
    render() {
        ...
        if (this.state.data.length > 0) {
            return (<Layout tableTitle="Vehicles"><DataTable data={this.state.data}
                orderBy="name"
                columnData={[
    ...
    async handleChangePage() {
        try {
            ...
            page = page + 1
            let data = await Service.getData("vehicles", page)
    ...
    async onRowClick(url) {
        try {
            let vehicle = await Service.getDetails(url)
