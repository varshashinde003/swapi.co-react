...
class People extends React.Component {
    ...
    async componentDidMount() {
        try {
            const data = await Service.getData("people", this.state.page)
    ...
    render() {
        ...
        if (this.state.data.length > 0) {
            return (<Layout tableTitle="People"><DataTable data={this.state.data}
                orderBy="name"
                columnData={[
                    { id: 'name', numeric: false, disablePadding: false, label: 'Name' },
                    { id: 'height', numeric: false, disablePadding: false, label: 'Height' },
                    { id: 'mass', numeric: false, disablePadding: false, label: 'Mass' },
                    { id: 'birth_year', numeric: false, disablePadding: false, label: 'Birth Year' },
            ...
            />
                ...
                >
                    ...
                    <PersonDescription person={this.state.person} />
                </Dialog>
            </Layout>
    ...
    async handleChangePage() {
        try {
            ...
            page = page + 1
            let data = await Service.getData("people", page)
            this.setState({ data: data.results, page }, () => console.log('[handleChangePage Inside people.js'))
        } catch (e) {
            console.error(e)
            throw e
        }
    ...
    async onRowClick(url) {
        try {
            let person = await Service.getDetails(url)
